...
#build cluster
result=$(curl -s -o /dev/null -u admin:admin -X POST -H 'X-Requested-By: ambari' -w "%{http_code}" ${ambariHost}/api/v1/blueprints/sandbox -d @sandbox.blueprint 2>/dev/null)
...
result=$(curl -s -o /dev/null -u admin:admin -X POST -H 'X-Requested-By: ambari' -w "%{http_code}" ${ambariHost}/api/v1/clusters/Sandbox -d @sandbox.hostmapping 2>/dev/null)
...
if [ $changePassword -eq 1 ]; then
   sed -i -- s/REPLACE-ME/$(curl http://169.254.169.254/latest/meta-data/instance-id 2>/dev/null)/g password.json
   result=$(curl -s -o /dev/null -u admin:admin -H "X-Requested-By: ambari" -w "%{http_code}" -i -X PUT http://${ambariHost}/api/v1/users/admin -d @password.json 2>/dev/null)
...
echo "****** Monitor progress via ambari"
if [ $changePassword -eq 1 ]; then
   echo "****** Ambari admin password is $(curl http://169.254.169.254/latest/meta-data/instance-id 2>/dev/null)"
fi
echo "***************************************************************"
